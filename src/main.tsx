...
const PLATFORM_GUIDES: Record<string, PlatformGuide> = {
  高通量反应平台: {
    ...
    chooseWhen: '用于筛选物料配比、试剂当量、催化剂或添加剂用量、溶剂量等。同一区内可分别设置各反应位的加粉量和加液量；温度和搅拌速度由反应区共享。',
    notFor: '不适用于百毫升级放大或复杂精制，也不能在同一区的 6 个反应位中分别设置不同温度、搅拌速度和后处理参数。若需筛选不同温度，应分别在不同反应区创建任务。',
    registryNote: '一个任务选择一个反应区，最多 6 组。4 个反应区可分别创建任务；共享机械臂的加料和转运动作按区依次执行。',
  },
  四釜反应平台: {
    purpose: '用于较大体积的制备反应和放大验证。支持固体与液体加料、控温、搅拌、气体置换或通气、控温滴加、冷凝回流、过程取样、液体回收和设备清洗。',
    chooseWhen: '百毫升级反应，或需要较完整过程控制的制备实验。',
    notFor: '小体积、多条件并行筛选；涉及萃取、过滤、旋蒸、柱层析等分离纯化操作时，应选择对应的后处理平台。',
  ...
  旋蒸预留平台: {
    ...
    registryNote: '支持 500 mL 或 1000 mL 旋蒸瓶，并要求填写油浴温度、时间、冷凝温度和真空度。当前用于实验流程和参数标注，不代表设备已经具备完整自动执行能力。',
  },
  过柱纯化平台: {
